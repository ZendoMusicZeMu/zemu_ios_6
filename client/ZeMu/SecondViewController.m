...
@interface SecondViewController () <UIWebViewDelegate, UIAlertViewDelegate>
...
- (void)viewDidLoad {
    ...
    self.radioModeButton = YES; // Кнопка "Радио" будет отображаться
    self.infoProgrammButton = YES; // Кнопка "Информация о программе" будет отображаться
    ...
    NSString *urlString = [NSString stringWithFormat:@"https://zendomusic.ru/ios6/music/?version=%@", appVersion];
...
- (void)checkPageTitle {
    ...
    if ([pageTitle isEqualToString:@"Музыка"]) {
